...
import { Gamepad2, Loader2, ShieldAlert, Sparkles } from "lucide-react";
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (authError) {
      ...
      if (data.session) {
        ...
        // If email confirmation is still enabled, they won't have a session yet.
        router.push("/login");
      }
    }
  };
  ...
  return (
    <div className="min-h-screen flex items-center justify-center p-4 sm:p-8 relative overflow-hidden bg-[#050505]">
      ...
      <div className="w-full max-w-4xl flex flex-col md:flex-row-reverse rounded-[2rem] overflow-hidden bg-black/20 backdrop-blur-2xl relative z-10 animate-fadeInUp shadow-[0_0_80px_rgba(0,112,209,0.3)] border border-white/10">
        ...
        {/* Right Side (visually): Branding/Hero Panel */}
        <div className="flex w-full md:w-5/12 p-6 sm:p-8 flex-col justify-center md:justify-between relative overflow-hidden text-white border-b md:border-b-0 md:border-l border-white/10 min-h-[200px] md:min-h-0">
          ...
          <div className="relative z-10 mt-6 md:mt-8 p-4 md:p-5 rounded-2xl bg-white/10 backdrop-blur-lg border border-white/20 shadow-xl animate-fadeInUp hidden sm:block" style={{ animationDelay: "1.5s" }}>
            ...
            <div className="mt-4 flex items-center gap-3">
              ...
              <div>
                <p className="text-xs font-bold text-white tracking-wider">MOHAMED FAHMY</p>
                <p className="text-[10px] text-white/60 tracking-widest uppercase">Premium User</p>
              </div>
            </div>
          </div>
        </div>
